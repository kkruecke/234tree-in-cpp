...
template<typename Key, typename Value> class tree234 {
   ...
   */
   ...
   Node *convert2Node(Node *node, int child_index) noexcept;
...
tree234<Key, Value>::get_delete_successor(Node *pdelete, Key delete_key, int delete_key_index) noexcept
{
  ...
  if (rightSubtree->isTwoNode()) { 
      ...
      convert2Node(rightSubtree, child_index);  
    /*
  ...
  Node *psuccessor = get_successor_node(rightSubtree); // <--- TODO: child_index is missing. Could convert2Node change child_index above?
 ...
 * 
 */
template<typename Key, typename Value> typename tree234<Key, Value>::Node *tree234<Key, Value>::convert2Node(Node *pnode, int child_index)  noexcept
{   
