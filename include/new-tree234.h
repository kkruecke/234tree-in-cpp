...
template<typename Key, typename Value> class tree234 {
   ...
   */
   ...
   std::pair<const Node *, int> getSuccessor(const Node *current, int key_index, int child_index) const noexcept;
   ...
   std::pair<const Node *, int> getLeafNodeSuccessor(const Node *pnode, int key_index, int child_index) const;
...
*/
template<class Key, class Value> std::pair<const typename tree234<Key, Value>::Node *, int> tree234<Key, Value>::getSuccessor(const Node *current, int key_index, int child_index) const noexcept
{
  if (current->isLeaf()) { // If leaf node
     ...
     if (current == root.get()) { // special case: current is root, which is a leaf      
        ...
        return getLeafNodeSuccessor(current, key_index, child_index);
     }
...
// TODO: Update child_index appropriately
template<class Key, class Value> std::pair<const typename tree234<Key, Value>::Node *, int> tree234<Key, Value>::getLeafNodeSuccessor(const Node *pnode, int key_index, int child_index) const 
{
...
template<class Key, class Value> inline tree234<Key, Value>::iterator::iterator(const iterator& lhs) : tree{lhs.tree}, current{lhs.current},\
        cursor{lhs.cursor}, key_index{lhs.key_index}, child_index{child_index}
{
}
...
template<class Key, class Value> inline tree234<Key, Value>::iterator::iterator(tree234<Key, Value>& lhs_tree, int i) :  tree{lhs_tree} 
{
  // If the tree is empty, there is nothing over which to iterate...
   if (!tree.isEmpty()) {
      ...
      child_index = cursor->parent->getTotalItems(); // Right most child
...
template<class Key, class Value> typename tree234<Key, Value>::iterator& tree234<Key, Value>::iterator::increment() noexcept	    
{
  ...
  auto [successor, index, child_index_tmp] = tree.getSuccessor(cursor, key_index, child_index);
...
template<class Key, class Value> typename tree234<Key, Value>::iterator& tree234<Key, Value>::iterator::decrement() noexcept	    
{
  ...
  auto [predecessor, index, child_index_tmp] = tree.getPredecessor(cursor, key_index, child_index);
