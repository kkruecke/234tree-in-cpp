...
template<typename Key, typename Value> inline  tree234<Key, Value>::Node::Node(__value_type<Key, Value>&& key_value) noexcept : parent{nullptr}, totalItems{1}
{
   // TODO: Confirm children are all nullptr.
...
{ 
  ...
  for(auto i = getTotalItems() - 1; i >= 0 ; --i) {
      ...
      if (lhs_key < key(i)) { // if key[i] is bigger
          ...
          keys_values[i].__ref() = std::make_pair<const key_type&, const mapped_type&>(lhs_key, lhs_value);
...
/*
 * Inserts vt_in pair into its sorted position in this Node and makes largerNode its right most child.
 */
/*++
...
template<typename Key, typename Value> typename tree234<Key, Value>::Node *tree234<Key, Value>::split(Node *pnode, Key new_key) noexcept
{
   ...
   if (root.get() == pnode) {
     ...
     // Insert pnode's keys_values[1] into its parent, and make largestNode its child.
