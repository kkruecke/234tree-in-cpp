...
template<typename Key, typename Value> bool tree234<Key, Value>::remove(Key key, const Node *current) 
{
   ...
   while(true) {
       ...
       if (current->SearchNode(key, key_index, child_index, next)) { // ...search for item in current node. 
           ...
           is_internal_node = !pfound_node->isLeaf();
...
/*
 * Gets the in-order successor of pfound_node->keys(key_index), the left-most leaf node in the subtree rooted at found_node->children[key_index + 1].
 * It converts any 2-nodes encountered into 3-nodes, and re-searches for the key if it moved during the conversion.
 * 
 */
