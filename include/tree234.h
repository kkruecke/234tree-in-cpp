        ...
        public:
          ...
          ~Node() // For debug purposes only
           ...
           class iterator : public std::iterator<std::forward_iterator_tag, KeyValue> {
                  ...
                  const Node& node;
                  int index;
                  iterator(const Node& n, bool end_flag) : node{n}
                  {
                       index = n.getTotalItems(); // called by end()
                  }

                public:
                  explicit iterator(const Node& n) : node{n}, index{0} {}

                  iterator(const iterator& it) : node{it.n}, index{it.index} {}

                  KeyValue& operator*()
                  {
                     return node.keys_values[index];
                  } 
                  iterator& operator++() 
                  { 
                    if (index != node.getTotalItems())  ++index;
                  }
                  ...
                  {
                      return (&node == &it.node && index == it.index) ? true : false;
                  } 
           ...
           class const_iterator : public std::iterator<std::forward_iterator_tag, const KeyValue>, private iterator {
                  ...
                  const_iterator(const Node& n, bool end_flag) : iterator{n, end_flag} {}

                public:
                  explicit const_iterator(const Node& n) : iterator{n} {}
...
{ 
   ...
   while(true) {
       ...
       if (auto [bool_found, next] = current->SearchNode(key); bool_found) {// return if key is already in tree
