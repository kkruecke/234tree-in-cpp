...
template<typename Key, typename Value> class tree234 {
 ...
 private:
   ...
   friend class DebugPrinter;
    ...
    const Node *getInternalNodeSuccessor_remove(Key key, const Node *current) noexcept; // new untested code
...
template<typename Key, typename Value> bool tree234<Key, Value>::remove(Key key, const Node *current) 
{
  ...
  if (!pfound_node->isLeaf()) {// The key is in an internal node, so we now search for its in order successor, converting any 2-nodes encountered.
      ...
      //++ getInternalNodeSuccessor_remove(key, current); replaces all code within this if-block.
 ...
 */
template<typename Key, typename Value> const typename tree234<Key, Value>::Node *tree234<Key, Value>::getInternalNodeSuccessor_remove(Key key, const Node *current) noexcept
{
   int child_index;
   int key_index;
   const Node *next = nullptr;
